...
function calculateDistance(movie1, movie2) {
  ...
  const calcIMDB_Rating = Math.pow(Number(movie1.IMDB_Rating) - Number(movie2.IMDB_Rating), 2)
  const calcMeta_score = Math.pow(Number(movie1.Meta_score) - Number(movie2.Meta_score), 2)
  const distance = Math.sqrt(calcIMDB_Rating + calcMeta_score)
  return 1 / (1 + distance)
}
...
const getMovieRecommendationV_1 = asyncWrapper(async (req, res) => {
  ...
  console.log(distances.map((d) => d.distance))

  const recommendations = distances.map((distance) => distance.movie).slice(0, 10)
